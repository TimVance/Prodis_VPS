...
class Order_admin extends Frame_admin
{
     ...
     * @return string
     */
    public function get_filter_variable_city()
    {
        $value = (! empty($this->diafan->get_nav_params['filter_city']) ? $this->diafan->get_nav_params['filter_city'] : array());

        $goods = DB::query_fetch_key_value("SELECT id, [name] FROM {shop_category} WHERE trash='0'", "id", "name");

        $res = $this->diafan->filter($_GET, 'string', "filter_city");

        $text = '<select name="filter_city"><option value="">Поиск по городу</option>';

        foreach($goods as $id => $name)
        {
            if(! $id)
                continue;
            $text .= '<option '.($res == $id ? 'selected ' : '').'value="'.$id.'">'.$name.'</option>';
        }

        $text .= '</select><div class="hr"></div>';
     ...
     * @return string
     */
    public function get_filter_variable_good()
    {
        $value = (! empty($this->diafan->get_nav_params['filter_good']) ? $this->diafan->get_nav_params['filter_good'] : array());

        $city = $this->diafan->filter($_GET, 'string', "filter_city");
        $where = '';
        if (!empty($city)) $where = ' AND s.cat_id='.$city;

        $goods = DB::query_fetch_key_value("SELECT s.id, s.[name] FROM {shop_order_goods} AS g RIGHT JOIN {shop} AS s ON g.good_id=s.id WHERE g.trash='0' AND s.trash='0'".$where." GROUP BY g.good_id", "id", "name");

        $res = $this->diafan->filter($_GET, 'string', "filter_good");

        $text = '<select name="filter_good"><option value="">Поиск по тц</option>';
     ...
     * @return mixed
     */
    public function save_filter_variable_good($row)
    {
        $res = '';
        if(! empty($_GET["filter_good"]) )
        {
            $res = $this->diafan->filter($_GET["filter_good"], 'integer');
        }
        if($res)
        {
            $this->diafan->join .= " LEFT JOIN {shop_order_goods} AS og ON e.id=og.order_id";
            $this->diafan->where .= " AND og.good_id='".$res."'";
            $this->diafan->get_nav .= ($this->diafan->get_nav ? '&amp;' : '?') . 'filter_good='.$res;
        }
        return $res;
    }
     ...
     * @return mixed
     */
    public function save_filter_variable_city($row)
    {
        $res = '';
        if(! empty($_GET["filter_city"]) )
        {
            $res = $this->diafan->filter($_GET["filter_city"], 'integer');
        }
        if($res)
        {
            $this->diafan->join .= " LEFT JOIN {shop_order_goods} AS goods ON e.id=goods.order_id";
            $this->diafan->join .= " LEFT JOIN {shop} AS shop ON goods.good_id=shop.id";
            $this->diafan->where .= " AND shop.cat_id='".$res."'";
            $this->diafan->get_nav .= ($this->diafan->get_nav ? '&amp;' : '?') . 'filter_city='.$res;
        }
        return $res;
    }
	...
	public function edit_variable_user_buy()
	{
		if ($this->diafan->values("user_id")>0)
		{
			...
			$orders = DB::query_result("SELECT COUNT(*) FROM {shop_order} WHERE user_id=%d AND trash='0'",$this->diafan->values("user_id"));
				if ($orders>1)
				{
					$user = $this->diafan->_('Всего заявок данного заказчика:').' <a href="'.BASE_PATH_HREF.'order/?filter_user_id='.$this->diafan->values("user_id").'">'.$orders .' '.'</a>';
				}
				else
				{
